...
def check(username, passwd, RealAddress, RealCity, RealCounty, RealProvince, BackState, MorningTemp, NightTemp):
	...
	print("登陆成功")
	# 提交信息
	response = commit_campus(commitUrl, RealAddress, RealCity, RealCounty, RealProvince, BackState, MorningTemp,
							 NightTemp, cookie)
	print(response)
...
def check_Job():
	...
	for t in range(3):
		...
		for i in range(1, len(users)):
			...
			users[i]
			try:
				res = check(username, passwd, RealAddress, RealCity, RealCounty, RealProvince, BackState, MorningTemp,
							NightTemp)
			except:
				res = -1
			cur = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
	...
	unCheck = ""
	for i in range(1, len(users)):
		username, passwd, email, RealAddress, RealCity, RealCounty, RealProvince, BackState, MorningTemp, NightTemp = users[
			i]
		cur = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
		print("错误", i)
